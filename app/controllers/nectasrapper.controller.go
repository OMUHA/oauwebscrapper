 ...
 func NectaCseeScrapper(ctx *fiber.Ctx) error {
		 ...
		 e.ForEach("table", func(_ int, el *colly.HTMLElement) {
			 if el.Attr("width") == "700" {
				 el.ForEach("a", func(_ int, el *colly.HTMLElement) {
					 ...
					 link := el.Attr("href")
					 centerNo := link[8:13]
					 if repository.CheckSchoolExists(centerNo) == false {
						 err := schoolResultCollector.Visit("https://matokeo.necta.go.tz/results2021/csee/"+link)
						 if err != nil {
							 fmt.Println(err)
							 return
						 }
	 ...
	 err := c.Visit("https://matokeo.necta.go.tz/results2021/csee/csee.htm")
	 if err != nil {
		 return err
	 }
